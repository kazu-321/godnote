...
export function getWorkspaceDeployWorkflowYaml() {
  return [
    ...
    "        run: npm ci",
    "      - name: Prepare viewer data",
    "        run: |",
    "          rm -rf dist/data",
    "          mkdir -p dist/data",
    "          cp -R workspace/manifest.json dist/data/manifest.json",
    "          cp -R workspace/subjects dist/data/",
    "          cp -R workspace/notes dist/data/",
    "      - name: Build viewer",
    "        run: VITE_VIEWER_BUILD=true npm run build",
