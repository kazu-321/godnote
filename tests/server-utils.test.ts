...
test("server path helpers point under sample workspace", () => {
  assert.match(dataRoot(), /sample_workspace$/);
  assert.equal(manifestPath().endsWith("sample_workspace/manifest.json") || manifestPath().endsWith("sample_workspace\\manifest.json"), true);
  assert.equal(subjectPath("subject-a").endsWith("sample_workspace/subjects/subject-a.json") || subjectPath("subject-a").endsWith("sample_workspace\\subjects\\subject-a.json"), true);
...
test("writeJsonFile creates directories and readJsonFile round-trips data", async () => {
  const root = await mkdtemp(join(tmpdir(), "godnote-json-"));
  try {
